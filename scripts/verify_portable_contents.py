...
def _validate_content_path(parts: tuple[str, ...], *, label: str) -> None:
    for index, part in enumerate(parts[:-1]):
        if part == "workflow_profiles" and not (index == 1 and parts[0] == "app"):
            raise ValueError(f"{label} contains forbidden directory: workflow_profiles")
    for index, part in enumerate(parts[:-1]):
        if part not in FORBIDDEN_DIRECTORY_NAMES:
            continue
        if part == "workflow_profiles" and index == 1 and parts[0] == "app":
            continue
